#!/usr/bin/env python3
"""
Seed script to populate sources table with 50+ news RSS feeds from around the world.
"""
import asyncio
import sys
...
async def seed_sources():
    ...
    async with async_session_maker() as session:
        for source in SOURCES:
            try:
                ...
                await session.execute(
                    text("""
                        INSERT INTO sources (name, feed_url, site_url, category_hint, language, crawl_difficulty, is_active)
                        VALUES (:name, :feed_url, :site_url, :category_hint, :language, :crawl_difficulty, true)
                    """),
                    source
                )
        ...
        print(f"\nDone! Added {len(SOURCES)} sources.")
